...
let processData = List.map (split >> parse) >> List.fold (fun (a,b)(c,d) -> a+c,b+d) (0,0) >> multTuple
...
let processData' = List.map (split >> parse) >> List.fold folder (0,0,0) >> (fun (a,b,_) -> a,b) >> multTuple
